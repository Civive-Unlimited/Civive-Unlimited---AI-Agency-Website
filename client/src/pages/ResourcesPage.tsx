...
import { pageMeta, resourceTopics } from "@/content/site";
          ...
          <div className="grid gap-12 lg:grid-cols-[0.68fr_1.32fr] lg:gap-16">
            ...
            <div className="divide-y divide-white/[0.08] border-y border-white/[0.08]">
              {resourceTopics.map((topic, index) => (
                <div key={topic.slug} className="grid gap-5 py-6 sm:grid-cols-[4rem_minmax(0,1fr)] sm:gap-8 sm:py-7">
                  <p className="font-mono text-xs uppercase tracking-[0.2em] text-white/36">
                    {String(index + 1).padStart(2, "0")}
                  </p>
                  <div>
                    <h2 className="text-xl font-medium text-white/92">{topic.title}</h2>
        ...
        <AuthoritySection>
          <div className="grid gap-8 lg:grid-cols-3 lg:gap-10">
            {contentLoops.map((loop) => (
              <div key={loop.title} className="border-y border-white/[0.08] py-7">
                <h2 className="text-2xl font-semibold text-white">{loop.title}</h2>
                <p className="mt-4 text-sm leading-6 text-white/62">{loop.copy}</p>
              </div>
            ))}
          </div>
        </AuthoritySection>
